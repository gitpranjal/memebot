...
class SocketIOOutput(OutputChannel):
    ...
    async def send_text_message(
        ...
        await self._send_message(self.sid, {"text": "  \n".join(text.split('\\n'))})
...
class MyIo(InputChannel):
    ...
    def blueprint(self, on_new_message):
        ...
        async def handle_message(sid, data):
            ...
            # re-formatting the user message to contain only single space between words
            logger.debug(f"Data frontend: {data}")
            custom_data = data['customData']
            frontend_message = data['message']

            # logger.debug(f"Message from frontend: {frontend_message}")
            # logger.debug(f"Custom data from frontend: {custom_data}")

            message = UserMessage(
                " ".join(frontend_message.split()), output_channel, sender_id, input_channel=self.name(), metadata=custom_data
            )
